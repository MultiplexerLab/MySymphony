...
package sym.appstore.universalvideoview;
...
public class UniversalMediaController extends FrameLayout {
    ...
    public UniversalMediaController(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        TypedArray a = mContext.obtainStyledAttributes(attrs, sym.appstore.R.styleable.UniversalMediaController);
        mScalable = a.getBoolean(sym.appstore.R.styleable.UniversalMediaController_uvv_scalable, false);
        a.recycle();
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        View viewRoot = inflater.inflate(sym.appstore.R.layout.uvv_player_controller, this);
    ...
    private void initControllerView(View v) {
        mTitleLayout = v.findViewById(sym.appstore.R.id.title_part);
        mControlLayout = v.findViewById(sym.appstore.R.id.control_layout);
        loadingLayout = (ViewGroup) v.findViewById(sym.appstore.R.id.loading_layout);
        errorLayout = (ViewGroup) v.findViewById(sym.appstore.R.id.error_layout);
        mTurnButton = (ImageButton) v.findViewById(sym.appstore.R.id.turn_button);
        mScaleButton = (ImageButton) v.findViewById(sym.appstore.R.id.scale_button);
        mCenterPlayButton = v.findViewById(sym.appstore.R.id.center_play_btn);
        mBackButton = v.findViewById(sym.appstore.R.id.back_btn);
        ...
        View bar = v.findViewById(sym.appstore.R.id.seekbar);
        ...
        mEndTime = (TextView) v.findViewById(sym.appstore.R.id.duration);
        mCurrentTime = (TextView) v.findViewById(sym.appstore.R.id.has_played);
        mTitle = (TextView) v.findViewById(sym.appstore.R.id.title);
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SHOW_LOADING: //3
                    show();
                    showCenterView(sym.appstore.R.id.loading_layout);
                    break;
                case SHOW_COMPLETE: //7
                    showCenterView(sym.appstore.R.id.center_play_btn);
                    break;
                case SHOW_ERROR: //5
                    show();
                    showCenterView(sym.appstore.R.id.error_layout);
                    break;
                case HIDE_LOADING: //4
                case HIDE_ERROR: //6
    ...
    private void showCenterView(int resId) {
        if (resId == sym.appstore.R.id.loading_layout) {
        ...
        } else if (resId == sym.appstore.R.id.center_play_btn) {
            if (mCenterPlayButton.getVisibility() != VISIBLE) {
        ...
        } else if (resId == sym.appstore.R.id.error_layout) {
            if (errorLayout.getVisibility() != VISIBLE) {
    ...
    public void reset() {
        ...
        mTurnButton.setImageResource(sym.appstore.R.drawable.uvv_player_player_btn);
    ...
    private void updatePausePlay() {
        if (mPlayer != null && mPlayer.isPlaying()) {
            mTurnButton.setImageResource(sym.appstore.R.drawable.uvv_stop_btn);
//            mCenterPlayButton.setVisibility(GONE);
        } else {
            mTurnButton.setImageResource(sym.appstore.R.drawable.uvv_player_player_btn);
//            mCenterPlayButton.setVisibility(VISIBLE);
    ...
    void updateScaleButton() {
        if (mIsFullScreen) {
            mScaleButton.setImageResource(sym.appstore.R.drawable.uvv_star_zoom_in);
        } else {
            mScaleButton.setImageResource(sym.appstore.R.drawable.uvv_player_scale_btn);
        }
    }
